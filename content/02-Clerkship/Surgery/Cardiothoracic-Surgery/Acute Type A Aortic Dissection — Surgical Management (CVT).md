---
title: "Acute Type A Aortic Dissection — Surgical Management (CVT)"
aliases: ["Acute Type A Aortic Dissection — Surgical Management (CVT)", "Acute Type A Aortic Dissection"]
stage: Clerkship
rotation: "Surgery"
...
**Short Note in CVT — C1 · Aorta (Template B: disease จบด้วย surgery).** หน้านี้ครอบคลุมเฉพาะ **surgical decision + operative concept + operative outcome** ของ acute type A aortic dissection (ATAAD). Disease-side (pathophysiology, Stanford/DeBakey classification, CTA, anti-impulse therapy, aneurysm threshold) อยู่ที่ [Acute Aortic Syndrome (AAS)](Acute%20Aortic%20Syndrome%20(AAS)%203a3224abad818117a02cd0341b321322.md) ไม่เขียนซ้ำที่นี่. Perioperative physiology (temp target, flow, ACT, cannulation detail) link ไป A3 DHCA & Cerebral Protection.
...
- **ห้ามใช้ clamp บน friable dissected aorta** distal anastomosis ต้องทำแบบ open ภายใต้ circulatory arrest (Class I) การ clamp เพิ่มความเสี่ยง rupture และ new tear
- **ห้าม drain pericardium เต็มปริมาณก่อน sternotomy ในราย tamponade** BP ที่ rebound ขึ้นหลัง decompression ขยาย dissection หรือ rupture ซ้ำ ให้ controlled drainage พอประคอง induction เท่านั้น
- **ห้ามตอบว่า axillary cannulation เหนือกว่า direct aortic แบบ absolute** 2024 guideline จัดสองอย่างระดับเดียวกัน key คือ antegrade flow + cerebral perfusion ไม่ใช่ site
- **ระวัง mesenteric malperfusion ที่ dead bowel** central repair บน bowel ที่ตายแล้วให้ mortality สูงมาก ต้องประเมิน viability และคุย sequencing กับ aortic team
</aside>
...
- **ATAAD = OR ทันที** — emergency surgery Class I; ขั้นต่ำ = ascending replacement + resect primary tear + open distal anastomosis under circulatory arrest
- **Root ladder:** resuspend + supracoronary graft (default) → David valve-sparing (leaflet ดี, อายุน้อย) → Bentall (root/valve เสีย, Marfan)
- **Arch:** hemiarch = standard; total arch/FET เฉพาะ arch tear หรือ extensive disease
- **Cannulation:** antegrade proximal-to-distal + cerebral perfusion สำคัญกว่า site; axillary = direct aortic (Class IIa)
- **Protection:** moderate hypothermia (~24–28°C) + ACP เป็น trend; deep = ≤20°C (นิยาม 2024)
- **Mortality:** STS ~14%, IRAD ~25% (unstable 31% / stable 17%); malperfusion ยกจาก ~6% เป็น ~22%
- **Pitfall:** mesenteric malperfusion = sequencing dilemma; residual false lumen = late distal reop (~15% in-hospital, 32–50% interval mortality)
</aside>
