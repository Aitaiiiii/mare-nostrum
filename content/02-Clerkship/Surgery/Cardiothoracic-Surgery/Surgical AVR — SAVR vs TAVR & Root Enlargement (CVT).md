---
title: "Surgical AVR — SAVR vs TAVR & Root Enlargement (CVT)"
aliases: ["Surgical AVR — SAVR vs TAVR & Root Enlargement (CVT)", "Surgical AVR"]
stage: Clerkship
rotation: "Surgery"
...
tags: []
created: 2026-08-04
notion_id: 3b2224ab-ad81-8159-a1a6-c5d21a72ff85
...
**Short Note in CVT — B3 · Ischemic & Valve (SNC-CVT Template B: disease→surgery).** เพจนี้ครอบ **SAVR operation + SAVR-vs-TAVR decision** — เน้น surgical angle. AS staging/pathophysiology → [AS (Aortic Stenosis) — Epidemiology, Pathophysiology, Staging & Management](AS%20(Aortic%20Stenosis)%20%E2%80%94%20Epidemiology,%20Pathophysiolo%203a0224abad8181fbb283cfb1d5914c25.md). TAVR device/complication → [TAVR (Transcatheter Aortic Valve Replacement) — Evidence, Valve Technology & Complications](TAVR%20(Transcatheter%20Aortic%20Valve%20Replacement)%20%E2%80%94%20Ev%203a0224abad818134a2c8cc2f1c7a011a.md). Valve choice (mech vs bio)/PPM/INR → [Prosthetic Heart Valves — Mechanical vs Bioprosthetic, PPM, Thrombosis & Selection Algorithm](Prosthetic%20Heart%20Valves%20%E2%80%94%20Mechanical%20vs%20Bioprosthe%203a0224abad818181b78bf696a7cb4e0f.md). Ross → B4.
...
SAVR ทำผ่าน **median sternotomy** (หรือ minimally invasive: upper hemisternotomy/right anterior thoracotomy) บน CPB ด้วย aortic cross-clamp + cardioplegia (→ [Myocardial Protection & Cardioplegia (CVT)](Myocardial%20Protection%20&%20Cardioplegia%20(CVT)%203b2224abad8181bd9584f6830ca76887.md)) — ขั้นตอนหลัก: transverse/hockey-stick aortotomy → ตัด native leaflet → decalcify annulus ให้เกลี้ยง (ระวัง debris embolization + AV block จากการทำลายลึกใกล้ conduction) → size annulus → เย็บ prosthesis ด้วย pledgeted suture → ปิด aortotomy + de-air
...
| Scenario | 2020 ACC/AHA | 2021 ESC/EACTS |
| --- | --- | --- |
| **อายุน้อย / low risk** | <65 ปี (หรือ life expectancy >20 ปี) → SAVR (Class I) | <75 ปี + low risk (STS/EuroSCORE II <4%) → SAVR (Class I) |
| **กลุ่มกลาง** | 65–80 ปี → SAVR หรือ TAVR (SDM, Heart Team, Class I) | ที่เหลือ → Heart Team ตาม clinical/anatomic/procedural |
...
- **อย่าเย็บ valve เล็กเกินใน small annulus โดยไม่คิด root enlargement** — severe PPM (iEOA ≤0.65) เทียบเท่า severe native AS, mortality ↑, ขัด reverse remodeling
- **อย่าเลือก TAVR ใน bicuspid + aortopathy โดยไม่ประเมิน** — ACC/AHA แนะ SAVR (ต้องจัดการ aorta ด้วย)
- **อย่า decalcify annulus ลึกเกินจนทะลุ conduction/annular rupture** — AV block (ต้อง pacemaker) หรือ annular tear
- **อย่าลืมว่า STS <4% ≠ อายุน้อย/durability ดี** — low surgical risk ไม่ได้แปลว่า valve จะทนนานในคนอายุน้อย (lifetime management)
</aside>
...
- **AV = replace ไม่ repair** (calcific leaflet); SAVR vs TAVR ตัดสินจาก age + surgical risk + anatomy + life expectancy vs durability
- **ACC/AHA 2020:** <65 → SAVR · 65–80 → SAVR/TAVR (SDM) · >80 → TAVR
- **ESC/EACTS 2021:** <75 + STS<4% → SAVR · >75 หรือ STS>8% → TAVR (75 = pivot)
- **SAVR ดีกว่าเมื่อ:** bicuspid+aortopathy, small/calcified annulus, concomitant CABG/valve/aorta, ไม่มี TF access
- **Root enlargement:** Nicks (~1 size) · Manouguian (~1–2, เสี่ยง MR) · Konno (LVOT) · Y-incision (2 size, รอง ViV-TAVR)
- **Pitfall:** small annulus ไม่ enlarge → severe PPM; valve choice/PPM/INR → Prosthetic Valves page
</aside>
