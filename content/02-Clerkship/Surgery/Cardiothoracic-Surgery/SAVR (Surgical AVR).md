...
**Short Note in CVT — B3 · Ischemic & Valve (SNC-CVT Template B: disease→surgery).** เพจนี้ครอบ **SAVR operation + SAVR-vs-TAVR decision** — เน้น surgical angle. AS staging/pathophysiology → [[Aortic Stenosis]]. TAVR device/complication → [[TAVR]]. Valve choice (mech vs bio)/PPM/INR → [[Prosthetic Heart Valves]]. Ross → B4.
...
SAVR ทำผ่าน **median sternotomy** (หรือ minimally invasive: upper hemisternotomy/right anterior thoracotomy) บน CPB ด้วย aortic cross-clamp + cardioplegia (→ [[Myocardial Protection & Cardioplegia]]) — ขั้นตอนหลัก: transverse/hockey-stick aortotomy → ตัด native leaflet → decalcify annulus ให้เกลี้ยง (ระวัง debris embolization + AV block จากการทำลายลึกใกล้ conduction) → size annulus → เย็บ prosthesis ด้วย pledgeted suture → ปิด aortotomy + de-air
