---
title: "CABG (Coronary Artery Bypass Grafting) — Surgical Revascularization (CVT)"
aliases: ["CABG (Coronary Artery Bypass Grafting) — Surgical Revascularization (CVT)", "CABG (Coronary Artery Bypass Grafting)"]
stage: Clerkship
rotation: "Surgery"
...
**Short Note in CVT — B1 · Ischemic & Valve (SNC-CVT Template A: procedure/operation).** หน้านี้ครอบคลุมเฉพาะ **surgical arm** ของ CABG — conduit choice, on/off-pump, completeness of revascularization, surgical complication และ surgical trial data. Disease-side (CCS pathophysiology, ischemia workup, medical therapy) อยู่ที่ [CCS (Chronic Coronary Syndromes / Chronic Coronary Disease)](CCS%20(Chronic%20Coronary%20Syndromes%20Chronic%20Coronary%20D%203a3224abad8181409b63f8992133584a.md). **CABG vs PCI decision + SYNTAX score selection** เป็นหน้าที่ของ B2 (SNC6-CAG/PCI) — ที่นี่แตะเฉพาะเท่าที่ trial ฝั่ง surgery พูด ไม่ทำ decision ladder ซ้ำ. Angiographic technique/lesion assessment อยู่ที่ [CAG & PCI (Coronary Angiography & Percutaneous Intervention)](CAG%20&%20PCI%20(Coronary%20Angiography%20&%20Percutaneous%20Int%203a4224abad81813c812de3765bb742bd.md).
...
- **LIMA (left internal mammary artery)→ LAD** เป็น gold standard สัมบูรณ์ patency >90% ที่ 10 ปี เพราะ internal mammary artery ต้านทาน atherosclerosis ได้ดีจาก endothelial biology (NO production สูง, internal elastic lamina หนา) การต่อ LIMA-LAD คือ single graft ที่ให้ survival benefit มากที่สุดและเป็น backbone ของทุก CABG
** ถ้า LM ตัน เราต่อ LIMA-LAD เส้นเดียว เชื่อว่าไป LCX ได้

contraindication — lerich syndrome
- **Radial artery** → ต่อ LCX — มักไปลง arterial conduit ที่ patency ดีกว่า saphenous vein เมื่อ target มี stenosis รุนแรง (>70–90%); RADIAL pooled analysis พบ clinical event ต่ำกว่า vein ต้องระวัง vasospasm ต้องให้ CCB
ข้อห้ามใช้ในคนอายุเยอะ เส้นจะไม่ดี
- **Gastroepiploic artery** ที่เลี้ยง stomach — ต่อ PDA
    
    <aside>
    
    best choice = 3 เส้น artery หมด
...
- **ห้ามละ LIMA→LAD** — เป็น single graft ที่ให้ survival benefit มากที่สุด ถ้าไม่มีเหตุจำเป็นจริง (LIMA เสีย, subclavian stenosis) ต้องต่อ LIMA เข้า LAD เสมอ
- **ห้ามใช้ BIMA ใน diabetic/obese/COPD แบบไม่ระวัง** — sternal devascularization + risk factor ทำให้ deep sternal wound infection พุ่ง; ART ไม่ได้พิสูจน์ว่า BIMA คุ้มความเสี่ยงนี้ใน ITT
- **ห้ามเลือก off-pump เพื่อ "ปลอดภัยกว่า" โดยไม่ดู expertise** — ROOBY แสดงว่า off-pump ในมือไม่ชำนาญให้ patency ต่ำและ revascularization ไม่ครบ ผลลัพธ์แย่กว่า on-pump
- **ระวัง incomplete revascularization** — เป็น driver ของ mortality ที่แย่กว่าตัวเลือก modality เอง ทั้ง PCI และ off-pump ที่ทำไม่ครบเสียเปรียบ complete CABG ชัดเจน
</aside>
...
- **LIMA→LAD = gold standard** patency >90% ที่ 10 ปี; single graft ที่ให้ survival benefit สูงสุด — ห้ามละ
- **Conduit patency ladder:** LIMA > radial (target stenosis รุนแรง) > SVG (~50–60% ที่ 10 ปี)
- **SYNTAX:** 3VD → CABG ชนะ (10-yr death 28% vs 21%); LM → เท่ากัน; ยิ่ง SYNTAX score สูงยิ่งหา CABG
- **FREEDOM:** DM + multivessel → CABG ชนะชัด (composite 18.7% vs 26.6%); Class I ในเบาหวาน
- **BIMA (ART):** 10-yr ITT ไม่ต่าง + sternal wound risk ↑ — เลี่ยงใน DM/obese
- **Off vs on-pump:** ROOBY (มือใหม่) off แย่กว่า; CORONARY (มือเก๋า) เท่ากัน → expertise-dependent, on-pump เป็น default
- **Completeness of revascularization** สำคัญกว่าตัว modality — incomplete PCI (rSS>8) 10-yr death 50% vs complete CABG 22%
- **Pitfall:** CABG แลก early stroke กับ long-term survival; benefit ใช้เวลา 4–5 ปีจึงปรากฏ
</aside>
